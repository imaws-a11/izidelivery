 ...
 if (!raw) return 'delivery';
  try {
    ...
    if (['restaurant', 'restaurante', 'food', 'hamburguer', 'hamburger', 'burger', 
         ...
         'churrasco', 'grill', 'culinaria', 'refeicao'].includes(t)) return 'restaurant';
    
    // Mercado / supermercado
    if (['market', 'mercado', 'supermercado', 'hortifruti'].includes(t)) return 'market';
    
    // Farmácia / saúde
    if (['pharmacy', 'farmacia', 'saude', 'health'].includes(t)) return 'pharmacy';
    ...
    if (['frete', 'carreto', 'freight', 'mudanca'].includes(t)) return 'frete';
    
    if (['motoboy', 'courier', 'moto', 'motoboy_express', 'entrega_avulsa', 'standalone', 'avulsa', 'avulso'].includes(t)) return 'motoboy';
