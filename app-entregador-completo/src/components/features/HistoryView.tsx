...
const HistoryView = React.memo<HistoryViewProps>(({ history, getNetEarnings, serviceTypeLabel, onSelectOrder }) => {
 ...
 estimateSize: () => 220, // Altura estimada de cada card
 overscan: 5,
 });

 return (
 <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="px-5 space-y-6 pt-4 h-full flex flex-col">
 <header className="flex flex-col items-center text-center gap-1 shrink-0">
 ...
 >
 {history.length === 0 ? (
 <div className="py-24 rounded-xl flex flex-col items-center gap-6 text-center relative overflow-hidden bg-white">
 <div className="size-20 rounded-full bg-zinc-50 border border-zinc-100 flex items-center justify-center ">
 ...
 className=" h-full rounded-xl p-6 space-y-5 relative overflow-hidden group cursor-pointer active:scale-[0.98] transition-all bg-white"
 >
 ...
 <span className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em] bg-zinc-50 px-3 py-1 rounded-full border border-zinc-100">
 #{order.id.slice(0, 8).toUpperCase()}
 </span>
 <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest flex items-center gap-1.5 drop-">
 <span className="size-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
 Finalizado
 </span>
 </div>

 <div className="bg-zinc-50 rounded-3xl p-5 border border-zinc-100 relative z-10">
 ...
 <div className="bg-zinc-50 p-4 rounded-xl border border-zinc-100 flex flex-col justify-between">
 ...
 <span className={`text-xl font-black tracking-tighter ${isNegative ? 'text-rose-600 drop-' : 'text-yellow-600 drop-'}`}>
 {netEarningsLabel}
 </span>
 </div>
 <div className="bg-zinc-100 p-4 rounded-xl border border-zinc-200 text-right flex flex-col justify-center ">
