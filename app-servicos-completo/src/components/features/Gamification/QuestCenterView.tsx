...
export const QuestCenterView = () => {
  ...
  const quests = [
    ...
    { id: 2, title: "Cliente Fiel",      desc: "Peça do mesmo restaurante 3 vezes",        xp: 100, progress: 66, icon: "favorite", color: "text-yellow-500", bg: "bg-yellow-100" },
    ...
    <div className="absolute inset-0 z-40 bg-white text-zinc-900 flex flex-col overflow-y-auto no-scrollbar pb-32">
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl flex items-center justify-between px-6 py-8 border-b border-zinc-100">
        <div className="flex items-center gap-5">
          ...
          <div>
            <h1 className="font-black text-2xl text-zinc-900 tracking-tighter uppercase leading-none">Quests & Ranking</h1>
      ...
      <main className="px-6 py-10 space-y-12">
        ...
        {/* XP PROGRESS CARD */}
        <section className="bg-zinc-50 p-8 rounded-[45px] border border-zinc-100 shadow-xl relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between relative z-10">
            <div className="space-y-1">
              ...
              <div className="flex items-baseline gap-2">
                 <h3 className="text-4xl font-black text-zinc-900 tracking-tighter">Nível {userLevel}</h3>
          ...
          <div className="mt-8 space-y-4 relative z-10">
            <div className="flex justify-between items-end">
              <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">XP Progress</span>
               <span className="text-sm font-black text-zinc-900 tabular-nums">{userXP} <span className="text-zinc-400 text-[10px] font-bold">/ {nextLevelXP}</span></span>
            </div>
            <div className="h-4 w-full bg-zinc-200 rounded-full p-1.5 shadow-inner border border-zinc-100 overflow-hidden">
              <motion.div 
                initial={{ width: 0 }} 
        ...
        <section>
          <div className="flex items-center justify-between mb-8 px-2">
             <h2 className="font-black text-xl text-zinc-900 tracking-tighter uppercase">Missões Ativas</h2>
          ...
          <div className="grid grid-cols-1 gap-5">
            {quests.map((q, i) => (
              <motion.div 
                ...
                className="bg-zinc-50 p-6 rounded-[35px] border border-zinc-100 shadow-md flex items-center gap-5 group"
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2 mb-1">
                    <p className="font-black text-sm text-zinc-900 tracking-tight leading-tight">{q.title}</p>
                  ...
                  <div className="h-1.5 w-full bg-zinc-100 shadow-inner rounded-full overflow-hidden border border-zinc-200">
                    <motion.div 
                      initial={{ width: 0 }} 
        ...
        <section>
          <div className="flex items-center justify-between mb-8 px-2">
             <h2 className="font-black text-xl text-zinc-900 tracking-tighter uppercase">Ranking Semanal</h2>
             <span className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">Cidade: São Paulo</span>
          </div>

          <div className="bg-zinc-50 rounded-[45px] border border-zinc-100 p-4 shadow-inner space-y-2">
             {ranking.map((row, i) => (
               <motion.div 
                 key={row.name || i}
               ...
               >
                  ...
                  <div className={`size-12 rounded-full flex items-center justify-center font-black text-sm border ${row.isMe ? 'bg-black/20 border-black/10 text-black' : 'bg-white border-zinc-200 text-zinc-900'}`}>
                     {row.avatar}
                  </div>
                  <div className="flex-1">
                     <p className={`font-black text-sm ${row.isMe ? 'text-black' : 'text-zinc-900'}`}>{row.name}</p>
