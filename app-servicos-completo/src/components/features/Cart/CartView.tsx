...
export const CartView: React.FC<CartViewProps> = ({ 
      ...
      >
        ...
        >
          ...
          <div className="text-center mb-12">
            <h2 className="text-[32px] font-black text-white uppercase tracking-tighter leading-none mb-5">
              Sacola <br/>
            ...
            <p className="text-[11px] text-zinc-500 font-black uppercase tracking-[0.3em] leading-relaxed opacity-80">
          ...
          <motion.button
            ...
            className="w-full h-16 rounded-[30px] bg-yellow-400 text-black font-black text-[13px] uppercase tracking-[0.2em] clay-button flex items-center justify-center gap-4 transition-all"
          >
            Explorar Menu
        ...
        {/* Bottom Tag */}
        <div className="absolute bottom-12 text-[9px] font-black text-zinc-800 uppercase tracking-[0.5em]">
  ...
  return (
    <div className="absolute inset-0 z-40 bg-black text-zinc-100 flex flex-col overflow-hidden select-none">
      ...
      <header className="z-[60] bg-black/60 backdrop-blur-2xl flex items-center justify-between px-6 py-7 border-b border-white/5">
        <div className="flex items-center gap-5">
          ...
          <div className="flex flex-col">
            <h1 className="font-black text-xl tracking-tight text-white uppercase leading-none">Minha Sacola</h1>
      ...
      <main className="flex-1 overflow-y-auto no-scrollbar px-6 flex flex-col pt-6 pb-44">
        ...
        {cart.length > 0 && (
          <section className="mt-2 space-y-4">
            <div className="flex items-center justify-between mb-4 px-2">
              <h2 className="text-[11px] font-black text-zinc-500 uppercase tracking-[0.3em]">Itens Selecionados</h2>
            ...
            {cart.map((item: any, i: number) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0 relative z-10">
                  ...
                  <h4 className="font-black text-lg text-white truncate leading-tight group-hover:text-yellow-400 transition-colors uppercase mb-2">{item.name}</h4>
        ...
        {suggestedMerchantProducts.length > 0 && (
          <section className="mt-14 mb-10">
             <div className="flex items-center justify-between mb-8 px-2">
                <h3 className="text-[12px] font-black text-white uppercase tracking-[0.25em]">Completa seu pedido?</h3>
             ...
             <div className="flex gap-6 overflow-x-auto no-scrollbar -mx-6 px-6 pb-6">
                {suggestedMerchantProducts.map((product: any) => (
                  <motion.div 
                    key={product.id || `prod-${product.id}`} 
                    whileTap={{ scale: 0.96 }}
                  ...
                  >
                     ...
                     <div className="px-2 pb-2">
                        ...
                        <h5 className="text-[14px] font-black text-white leading-tight uppercase truncate mb-4">{product.name}</h5>
                        <div className="flex items-center justify-between">
                           <span className="text-base font-black text-white tracking-tighter">R$ {Number(product.price || 0).toFixed(2).replace(".", ",")}</span>
        ...
        {cart.length > 0 && (
          <section className="mt-4 space-y-5 p-7 bg-zinc-900/30 rounded-[45px] border border-white/5 clay-card mb-8">
            <div className="flex items-center justify-between px-1">
              <span className="text-zinc-500 text-xs font-black uppercase tracking-[0.2em]">Subtotal</span>
              <span className="text-white font-black text-sm tracking-tight">R$ {subtotal.toFixed(2).replace(".", ",")}</span>
            </div>
            
            <div className="flex items-center justify-between px-1">
              <span className="text-zinc-500 text-xs font-black uppercase tracking-[0.2em]">Taxa de entrega</span>
              <div className="flex flex-col items-end">
                 {taxa === 0 ? (
                   <div className="flex flex-col items-end">
                     <span className="text-emerald-400 font-black text-xs tracking-widest uppercase bg-emerald-400/10 px-3 py-1 rounded-lg border border-emerald-400/10">Grátis</span>
                     {isIziBlack && (
                       <span className="text-[8px] text-zinc-600 font-black tracking-[0.2em] uppercase mt-1.5">Benefício Izi Black</span>
                     )}
                   </div>
                 ) : (
                   <span className="text-white font-black text-sm tracking-tight">R$ {taxa.toFixed(2).replace(".", ",")}</span>
                 )}
              </div>
            </div>
            ...
            <div className="p-5 rounded-[32px] bg-zinc-950/50 border border-white/5 flex items-center justify-between group overflow-hidden relative clay-card">
              ...
              <div className="flex items-center gap-4 relative z-10">
                <div className="size-11 rounded-2xl bg-yellow-400 text-black flex items-center justify-center font-black text-xl clay-card-yellow shadow-[0_0_20px_rgba(255,215,9,0.2)]">Z</div>
                <div className="flex flex-col">
                  <span className="text-[9px] font-black text-yellow-400 uppercase tracking-widest leading-none mb-1.5">Ganhe agora</span>
                  <span className="text-white font-black text-sm tracking-tight uppercase">{((subtotal * (isIziBlack ? (iziBlackRate || 0) : (iziCoinRate || 0))) / 100).toLocaleString('pt-BR', { minimumFractionDigits: 0, maximumFractionDigits: 3 })} IZI COINS</span>
                </div>
              </div>
              {isIziBlack ? (
                <div className="flex flex-col items-end relative z-10">
                  <span className="text-[8px] font-black text-zinc-600 uppercase tracking-widest mb-1.5">Cashback Black</span>
                  <span className="text-emerald-400 font-black text-xs tracking-widest uppercase">{iziBlackRate || 0}%</span>
                </div>
              ) : (
                <button 
                  onClick={() => navigateSubView("izi_black_purchase")}
                  className="relative z-10 px-5 py-2.5 rounded-2xl bg-yellow-400 text-black text-[10px] font-black uppercase tracking-widest clay-card-yellow active:scale-90 transition-all shadow-[0_10px_20px_rgba(250,204,21,0.2)]"
                >
                  Assinar Black
                </button>
              )}
            ...
            <div className="pt-6 mt-2 border-t border-white/5 flex justify-between items-end px-1">
              <div className="flex flex-col gap-3">
                 <p className="text-zinc-500 text-[10px] font-black uppercase tracking-[0.3em] font-black opacity-60">Entrega Estimada</p>
                 <div className="flex items-center gap-3 bg-zinc-950/50 px-4 py-2 rounded-2xl border border-white/5 clay-card w-fit">
                    ...
                    <span className="font-black text-white text-xs uppercase tracking-tight">35 - 45 min</span>
                 </div>
              </div>
               <div className="text-right">
                 <p className="text-zinc-500 text-[10px] font-black uppercase tracking-[0.3em] mb-1 opacity-60">Total Final</p>
                 <p className="text-4xl font-black text-white leading-none tracking-tighter" style={{ textShadow: "0 0 30px rgba(255,255,255,0.15)" }}>
        ...
        {cart.length > 0 && (
          ...
          >
            ...
            <span className="text-[11px] font-black uppercase tracking-[0.3em]">Esvaziar Sacola</span>
          </button>
        )}
      </main>
      ...
      {cart.length > 0 && (
        <div className="fixed bottom-0 left-0 w-full px-6 pb-12 pt-4 z-[100]">
          <button
            onClick={() => navigateSubView("checkout")}
            className="w-full h-20 rounded-[35px] bg-yellow-400 flex items-center justify-between px-10 transition-all active:scale-[0.97] relative overflow-hidden group"
          >
            <div className="flex flex-col items-start relative z-10">
               <h3 className="text-black font-black text-xl uppercase tracking-[0.1em] leading-none">Continuar</h3>
            </div>
            
            <div className="flex items-center gap-6 relative z-10">
               <div className="h-6 w-px bg-black/10" />
               <div className="flex flex-col items-end">
                  <span className="text-black font-black text-2xl tracking-tighter leading-none">R$ {total.toFixed(2).replace(".", ",")}</span>
               </div>
