...
export const MerchantCard = ({ shop, onClick, index }: MerchantCardProps) => {
  return (
    <motion.div
      ...
      className={`group bg-zinc-800 p-4 rounded-[35px] shadow-[10px_10px_20px_rgba(0,0,0,0.4),-5px_-5px_15px_rgba(255,255,255,0.02),inset_4px_4px_8px_rgba(255,255,255,0.03),inset_-4px_-4px_8px_rgba(0,0,0,0.4)] transition-all flex items-center gap-4 cursor-pointer relative overflow-hidden mb-2 ${!shop.isOpen ? 'opacity-60 grayscale' : ''}`}
    >
      {/* Glossy Overlay */}
      ...
      <div className="flex-1 min-w-0 relative z-10">
        <div className="flex items-center justify-between gap-2 mb-2">
          <div className="flex flex-col min-w-0">
            ...
            <div className={`mt-1.5 flex items-center gap-1.5 px-2 py-0.5 rounded-full border w-fit ${shop.isOpen ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' : 'bg-rose-500/10 border-rose-500/20 text-rose-500'}`}>
               <div className={`size-1 rounded-full ${shop.isOpen ? 'bg-emerald-500 shadow-[0_0_8px_#10b981]' : 'bg-rose-500'} animate-pulse`} />
               <span className="text-[8px] font-black uppercase tracking-widest">{shop.isOpen ? 'Aberto Agora' : 'Fechado'}</span>
            </div>
          </div>
        ...
        <div className="flex items-center gap-3">
          <motion.div 
            className={`flex items-center justify-center px-4 py-2 rounded-2xl ${shop.isOpen ? 'bg-yellow-400' : 'bg-zinc-700'} shadow-[4px_4px_8px_rgba(0,0,0,0.3),inset_2px_2px_4px_rgba(255,255,255,0.4),inset_-2px_-2px_4px_rgba(0,0,0,0.2)] transition-all`}
          >
      ...
      <div className={`size-11 rounded-2xl bg-zinc-900 shadow-[4px_4px_8px_rgba(0,0,0,0.4),inset_2px_2px_4px_rgba(255,255,255,0.02),inset_-2px_-2px_4px_rgba(0,0,0,0.5)] flex items-center justify-center shrink-0 ${shop.isOpen ? 'group-hover:bg-yellow-400 group-hover:shadow-[0_0_15px_rgba(251,191,36,0.3)]' : 'opacity-30'} transition-all duration-500`}>
        <span className={`material-symbols-outlined text-[18px] ${shop.isOpen ? 'text-zinc-600 group-hover:text-black' : 'text-zinc-700'} font-black`}>arrow_forward_ios</span>
      </div>
    </motion.div>
  );
};
