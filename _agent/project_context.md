...
- **Atualização Inteligente de Dossiê**: O sistema identifica automaticamente quais dos 5 documentos estão nulos no banco de dados e apresenta uma tela de atualização parcial (`update_docs`) focada apenas nos itens pendentes. Motoristas ativos com documentos incompletos são retidos nesta tela com design Glassmorphism Stealth Luxury até a regularização.
