...
- **App Admin (`StandaloneDeliveryTab.tsx`)**: Payload agora inclui `pickup_address`, `pickup_lat`, `pickup_lng`, `store_name` e `payment_status: 'paid'` (taxa debitada do saldo do lojista).
- **App Entregador (`App.tsx`)**: 
  - `normalizeServiceType`: `'entrega_avulsa'` mapeado para `'motoboy'`.
  - `getCategory`: inclui `'entrega_avulsa'` na categoria `'motoboy'`.
  - Realtime `isCompatible`: `'entrega_avulsa'` reconhecido como `isDelivery`.
  - `isPaidOrCash` / `shouldSound`: trata `entrega_avulsa` como pedido pago (dispara som/popup).
  - `isPaid` (ActiveMission, OrderDetails, finalizador): inclui `service_type === 'entrega_avulsa'` para não solicitar pagamento ao entregador.
  - Card "Novos Pedidos": Badge verde "📦 ENTREGA AVULSA", título e subtítulo customizados, label "Taxa paga pela loja".
  - `getServicePresentation`: headline "Entrega Avulsa" quando `rawType` é `entrega_avulsa`.
