...
Responda em português, máx 3 linhas, use emojis com moderação.`;
  ...
  const sendMessage = async () => {
    ...
    try {
      // Chamada segura via Edge Function — a chave Anthropic fica no servidor
...
function App() {
  ...
  const fetchWalletBalance = async (uid: string) => {
    const { data, error } = await supabase
      ...
      .eq("id", uid)
      .maybeSingle(); // maybeSingle evita erro 406 quando o registro não existe ainda

    if (error) {
      console.warn("[fetchWalletBalance] Erro ao buscar perfil:", error.message);
    }
    ...
    if (data) {
      ...
      // Usuário autenticado mas sem registro em users_delivery — criar automaticamente
      console.log("[fetchWalletBalance] Perfil não encontrado, criando via upsert...");
